		...
		try (Scanner scanner = new Scanner(System.in)) {
			do {
				...
				case 1:
					...
					break;
				case 2:
					OwnerPetPrimaryKey ownerPetPrimaryKey = InputUtil.acceptCustomPrimaryKeyToOperate(scanner);
					ownerDTO = ownerService.findOwner(ownerPetPrimaryKey);
					System.out.println(properties.getProperty(OWNER_FOUND));
					System.out.println(ownerDTO);
					break;
				case 3:
					ownerPetPrimaryKey = InputUtil.acceptCustomPrimaryKeyToOperate(scanner);
					String petName = InputUtil.acceptPetDetailsToUpdate(scanner);
					ownerService.updatePetDetails(ownerPetPrimaryKey, petName);
					System.out.println(properties.getProperty(OWNER_UPDATED));
					break;
				case 4:
					ownerPetPrimaryKey = InputUtil.acceptCustomPrimaryKeyToOperate(scanner);
					ownerService.deleteOwner(ownerPetPrimaryKey);
