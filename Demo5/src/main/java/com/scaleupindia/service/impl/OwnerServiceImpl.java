...
public class OwnerServiceImpl implements OwnerService {
	...
	public void saveOwner(OwnerDTO ownerDTO) throws DuplicateOwnerException {
		...
		if (Objects.nonNull(existingOwner)) {
			throw new DuplicateOwnerException(properties.getProperty(OWNER_ALREADY_EXISTS) + primaryKey);
		}
	...
	@Override
	public OwnerDTO findOwner(OwnerPetPrimaryKey ownerPetPrimaryKey) throws OwnerNotFoundException {
		Owner owner = ownerRepository.findOwner(ownerPetPrimaryKey);
		if (Objects.isNull(owner)) {
			throw new OwnerNotFoundException(properties.getProperty(OWNER_NOT_FOUND) + ownerPetPrimaryKey);
		}
		return MapperUtil.convertOwnerEntityToDto(owner);
	}

	@Override
	public void updatePetDetails(OwnerPetPrimaryKey ownerPetPrimaryKey, String petName) throws OwnerNotFoundException {
		Owner owner = ownerRepository.findOwner(ownerPetPrimaryKey);
		if (Objects.isNull(owner)) {
			throw new OwnerNotFoundException(properties.getProperty(OWNER_NOT_FOUND) + ownerPetPrimaryKey);
		}
		ownerRepository.updatePetDetails(ownerPetPrimaryKey, petName);
	}

	@Override
	public void deleteOwner(OwnerPetPrimaryKey ownerPetPrimaryKey) throws OwnerNotFoundException {
		Owner owner = ownerRepository.findOwner(ownerPetPrimaryKey);
		if (Objects.isNull(owner)) {
			throw new OwnerNotFoundException(properties.getProperty(OWNER_NOT_FOUND) + ownerPetPrimaryKey);
		}
		ownerRepository.deleteOwner(ownerPetPrimaryKey);
	}
}
