		...
		try (Scanner scanner = new Scanner(System.in)) {
			do {
				...
				case 5:
					...
					ownerDTOList = ownerService.findAllOwnersV2();
					System.out.println("There are " + ownerDTOList.size() + " owners.");
					ownerDTOList.forEach(System.out::println);
					break;
				case 6:
					List<Integer> ownerIdList = InputUtil.acceptOwnerIdsToOperate(scanner);
					ownerDTOList = ownerService.findAllOwnersWithIds(ownerIdList);
					System.out.println("There are " + ownerDTOList.size() + " owners with Primary Caching enabled.");
					ownerDTOList.forEach(System.out::println);
					ownerDTOList = ownerService.findAllOwnersWithIdsV2(ownerIdList);
